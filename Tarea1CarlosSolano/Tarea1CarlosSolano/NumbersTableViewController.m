...
//  Tarea1CarlosSolano
//
//  Created by Carlos Solano on 8/2/17.
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.text = [NSString stringWithFormat:@"8 * %ld =",indexPath.row];
    cell.detailTextLabel.text = self.multiplicationArray[indexPath.row];
...
-(void)loadData{
    self.multiplicationArray = [NSMutableArray new];
    int number = 8;
    for (int i=0; i <= 100; i++) {
        int total = number * i;
        NSString *totalStr = [NSString stringWithFormat:@"%d",total];
        [self.multiplicationArray addObject:totalStr];
    }
}
